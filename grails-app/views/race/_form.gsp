...
<div class="fieldcontain ${hasErrors(bean: raceInstance, field: 'state', 'error')} ">
    ...
    <g:select name="state" from="${raceInstance.constraints.state.inList}" value="${raceInstance?.state}"
              valueMessagePrefix="race.state" noSelection="['': '']"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: raceInstance, field: 'distance', 'error')} required">
    ...
    <g:field type="number" name="distance" step="0.01" min="0.0" required=""
             value="${fieldValue(bean: raceInstance, field: 'distance')}"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: raceInstance, field: 'cost', 'error')} required">
    ...
    <g:field type="number" name="cost" step="0.01" min="0.0" max="100.0" required=""
             value="${fieldValue(bean: raceInstance, field: 'cost')}"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: raceInstance, field: 'maxRunners', 'error')} required">
    ...
    <g:field type="number" name="maxRunners" min="0" max="100000" required=""
             value="${fieldValue(bean: raceInstance, field: 'maxRunners')}"/>
</div>
    ...
    <ul class="one-to-many">
        ...
        <li class="add">
            <g:link controller="registration" action="create"
                    params="['race.id': raceInstance?.id]">${message(code: 'default.add.label', args: [message(code: 'registration.label', default: 'Registration')])}</g:link>
        </li>
    </ul>
